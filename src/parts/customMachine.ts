...
import { useNuxt, useLogger, addTemplate, addImportsDir } from '@nuxt/kit'
...
export const setupCustomMachines = async (machinesOptions: CustomMachinesOptions) => {
  ...
  addImportsDir(resolvedBuildDir)
  ...
  nuxt.hook('builder:watch', async (_, path) => {
    ...
    if (machineDirAffected && isJsOrTs) {
...
const handleAutoImport = async (options: {
  ...
  const logger = useLogger('nuxt-xstate')
  ...
  logger.info(
    moduleStarting
      ...
      : 'Change in the state machine directory detected, updating imports...'
  )
